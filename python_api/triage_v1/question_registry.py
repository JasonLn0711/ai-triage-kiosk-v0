...
class QuestionRegistry:
    def __init__(self, csv_path: Path, seed_questions: list[Question] | None = None) -> None:
        ...
        with self.csv_path.open(encoding="utf-8", newline="") as csv_file:
            for row in csv.DictReader(csv_file):
                ...
                question = Question(
                    ...
                    type=_question_type(row.get("question_type") or ""),
                    text=text,
                    options=options,
                    max_selections=max(1, len(options)),
                    trigger_reason_codes=[f"module_{question_id.lower()}"],
                    source_refs=[row.get("module_file") or "Case_question/symptom_questions.csv"],
                ...
                module_key = (row.get("module_file") or "").strip()
