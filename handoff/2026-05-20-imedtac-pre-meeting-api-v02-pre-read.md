...
Post-baseline update: 慧誠在 `2026-05-12` 已提供 iMVS API `V1.4`，其中
`NBP/SPO2/HR/Temp/Glucose/Height/Weight` 與 sample units 可作為 adapter
baseline。NYCU 可以先依照以下 shape 製作 API v0.2 草稿；慧誠工程團隊需確認
current demo machine / GitHub 格式是否與 V1.4 有 field-name delta、required /
optional 狀態差異，以及 missing / failure 語意。
...
- 確認 current iMVS vital field names / units 是否沿用 5/12 V1.4 baseline，
  以及是否有 delta。
...
| 負責人 | 交付項目 | 建議期限 | 驗收方式 |
| --- | --- | --- | --- |
| 慧誠 engineering | Synthetic iMVS vital payload example 與 current field-dictionary delta | 2026-05-22 | 確認是否沿用 5/12 V1.4 欄位與 units，並明確化 required / optional、missing / failure behavior。 |
