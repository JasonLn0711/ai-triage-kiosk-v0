...
圖 1 顯示本提案之第 1 層系統脈絡。病人於門診前或候診期間完成問卷、報到或量測；系統接收問卷、既有健康報告、量測資料與候診隊列資訊後，產生醫護審閱摘要與檢查候選提示。醫師保留是否核准檢查單之決定權；核准後之檢查單與檢查安排，透過既有 HIS、開單系統、LIS 或 RIS 等外部系統完成。

本圖採第 1 層系統脈絡揭露範圍，保留 C4 第 2 層容器圖、第 3 層元件圖、第 4 層程式碼、模型服務、資料庫配置、提示鏈、分數門檻、路由權重、部署方式或醫院特定串接細節，供後續受控技術揭露與專利工程判斷使用。
