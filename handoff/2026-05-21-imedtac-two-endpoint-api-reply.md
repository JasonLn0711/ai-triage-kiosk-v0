...
4. Demo environment
   ...
   - Demo bearer token 採 NYCU 環境變數控制做法：若 NYCU 在 Render 設定
     ...
     Token-required mode 已在 Render redeploy 後生效；public check 目前確認
     no-token POST 會回 HTTP `401` / `demo_bearer_token_required`，
     bearer-token 成功路徑則透過私下 token 測試。
...
5. Summary preview page
   ...
   - 若 imedtac 需要先快速驗證畫面，NYCU 可以提供輕量 NYCU-hosted demo
     ...
     因為 NYCU-provided UI 可能影響畫面一致性與設備操作完整性，summary
     display surface 不應單方面切換，需先與 imedtac 討論。
