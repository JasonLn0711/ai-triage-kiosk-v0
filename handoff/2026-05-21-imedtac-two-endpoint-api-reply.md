...
下列表格最後一欄提供每個 field 的實際 demo JSON 範例與簡短說明。範例值以六月
tachycardia live lane 與 respiratory fallback lane 的 synthetic demo payload 為主，
方便 imedtac 工程團隊直接對照 request / response shape。
...
| `patient_context.age` | number | no | synthetic demo 年齡；只供 demo case 情境使用。 | 範例：`45`；供 demo case 呈現成人情境。 |
| `patient_context.sex` | string | no | synthetic demo 生理性別或情境性別；只供 demo case 情境使用。 | 範例：`male`；供 demo case 內容對齊。 |
| `vitals` | object | yes | iMVS 量測完成後送給 NYCU 的 measured 或 synthetic vital payload。 | 範例：`{"heart_rate_bpm":{"value":118}}`；NYCU 用此 object 產生 vital-aware 問題。 |
...
| `vitals.<field>.value` | number/null | 單一 vital 欄位的量測值；若 unavailable、failed 或不適用，可為 `null`。 | 範例：`heart_rate_bpm.value = 118`；代表心跳量測值。 |
| `vitals.<field>.unit` | string | 單位；例如 `%`、`C`、`mmHg`、`beats/min`、`cm`、`kg`。 | 範例：`beats/min`；讓 iMVS 與 NYCU 對心跳單位有一致理解。 |
...
| `question` | object | yes when `status=question` | NYCU 回傳給 iMVS 顯示的 typed question object。 | 範例：`{"id":"chief-concern","type":"single_choice"}`；iMVS 依此 render 第一題。 |
...
| 欄位 | 型別 | 必填 | 定義 | 範例與說明 |
| --- | --- | --- | --- | --- |
| `question.id` | string | yes | 穩定的 runtime question id；iMVS 回答時以此值作為 `question_id`。 | 範例：`chief-concern`；iMVS submit answer 時以此值作為 `question_id`。 |
| `question.registry_refs` | array | yes | 對應到 NYCU 內部 question registry 的來源 id；用於 trace 問題來源與版本。 | 範例：`["NYCU-TACHY-Q-001"]`；代表此題對應內部題庫 registry。 |
...
| `question.options` | array | yes | 選項清單；每個 option 應包含穩定 `id` 與顯示 `label`。 | 範例：`[{"id":"chest_discomfort","label":"Chest discomfort"}]`；iMVS 用 `label` 顯示、用 `id` 回傳答案。 |
...
| `question_id` | string | yes | 使用者正在回答的 question id；應等於 NYCU 前一個 response 的 `question.id`。 | 範例：`chief-concern`；NYCU 用此值把答案綁到前一題。 |
| `answer.selected_option_ids` | array | yes for choice questions | 使用者選取的 option id 清單；單選題通常只有一個 id，複選題可有多個 id。 | 範例：`["chest_discomfort"]`；代表使用者選了胸口不適選項。 |
...
| `handoff_reason_codes` | array | yes | 需要人工 review 的機器可讀理由代碼，例如 `vital_review`、`reported_symptoms_review`。 | 範例：`["vital_review","reported_symptoms_review"]`；前端或 log 可用此值標記 handoff 原因。 |
...
| `staff_review_summary.subjective` | array | yes | 使用者回報的主觀資訊，例如主訴、症狀、使用者選項。 | 範例：`["Synthetic demo patient selected chest discomfort."]`；整理使用者選項。 |
| `staff_review_summary.objective` | array | yes | iMVS 量測到的客觀資訊，例如 heart rate、SpO2、temperature 等 vital payload 摘要。 | 範例：`["Measured heart rate is available in the demo vital payload."]`；整理 iMVS 量測資料。 |
| `staff_review_summary.review_basis` | array | yes | 支援 staff review 的資訊依據；欄位語意維持在 review context。 | 範例：`["Measured vitals and reported symptoms should be reviewed by staff."]`；說明 summary 依據。 |
| `staff_review_summary.review_action` | array | yes | 給 staff 的 review reminder 與 workflow cue。 | 範例：`["Please review measured vitals and reported symptoms."]`；提供 staff workflow cue。 |
| `staff_review_summary.staff_handoff_note` | string | yes | 給 staff / doctor preview 的短句，提醒檢視量測數值與使用者回報症狀。 | 範例：`Please review measured vitals and reported symptoms.`；可直接顯示在 preview。 |
