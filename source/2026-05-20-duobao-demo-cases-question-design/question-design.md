...
- Suggested Acuity: <Emergent / Urgent / Non-urgent>
```

### Plan (P)

```
- Suggested Disposition: <ER / Urgent Care / Clinic / Home>
- Recommended Department: <e.g., Cardiology, Surgery, Internal Medicine>
- Immediate Actions: <e.g., Notify staff, Remeasure vitals, ECG if chest pain>
```
...
```mermaid
graph TD
    A[Patient Arrival] --> B[Initial Phase - Section 0]
    B --> C[Symptom-Specific Phase - Section 1]
    
    %% The Parallel / Interrupt Logic
    C -->|Abnormal Vitals Detected| E[After Vital Signs Phase - Section 3]
    C -->|Normal Vitals| D[Universal Phase - Section 2]
    
    E --> D
    D --> F[Generate SOAP Report]
    F --> G[Triage Complete]
```
